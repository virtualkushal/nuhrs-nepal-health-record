"""
httpOnly-cookie JWT transport for the SwasthyaEHR backend.

simplejwt 5.3.1 ships no cookie transport, so this small module adds one:

  * Access + refresh JWTs live in httpOnly cookies (``swasthya_access_token`` /
    ``swasthya_refresh_token``) instead of the JSON response body, so client-side
  ...
  * SameSite=Lax cookies plus the SPA's same-origin ``/api`` proxy are the
    primary CSRF defense; the CSRF token is defense-in-depth. ``Secure`` is set
    automatically whenever DEBUG is off (i.e. everywhere but the local demo).

Login / refresh set the cookies; logout clears them.
"""
...
ACCESS_COOKIE = "swasthya_access_token"
REFRESH_COOKIE = "swasthya_refresh_token"
...
def _cookie_flags():
    ...
    # because the SPA is served same-origin with its API (via an /api proxy),
...
def set_auth_cookies(response, access=None, refresh=None):
    ...
    if access is not None:
        response.set_cookie(
            ACCESS_COOKIE,
            access,
            max_age=_lifetime_seconds("ACCESS_TOKEN_LIFETIME", 60 * 60),
            **flags,
        )
    if refresh is not None:
        response.set_cookie(
            REFRESH_COOKIE,
            refresh,
            max_age=_lifetime_seconds("REFRESH_TOKEN_LIFETIME", 7 * 24 * 3600),
            **flags,
        )
    return response
...
class CookieJWTAuthentication(JWTAuthentication):
    """Authenticate from the ``swasthya_access_token`` cookie.
