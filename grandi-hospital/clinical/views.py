"""
Local hospital data-entry API (used by Norvic staff UI).

Creating a Condition/Observation also pushes a metadata pointer to the National
Platform so the record becomes discoverable nationwide.
"""
...
from .models import Condition, LocalPatient, Observation
...
def _split_name(full):
    """Variant B stores name as first + last; split a supplied full name."""
...
def create_condition(request):
    ...
    condition = Condition.objects.create(
        patient=patient, condition_desc=text, icd_code=code,
        status="active", onset=onset,
    )
