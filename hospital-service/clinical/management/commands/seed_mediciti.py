"""
Rich, realistic seed for Nepal Mediciti Hospital (variant A / HOSP001).

Builds a full hospital record for 6 patients across 8 departments:
...
Run:  python manage.py seed_mediciti
"""
...
class Command(BaseCommand):
    help = "Rich, idempotent seed for Nepal Mediciti Hospital (variant A)."

    def handle(self, *args, **options):
        if settings.ORG_CODE != "HOSP001":
            self.stdout.write(self.style.WARNING(
                f"seed_mediciti is only for HOSP001 (Mediciti). This is {settings.ORG_CODE}. Skipping."))
            return
        if settings.SCHEMA_VARIANT != "A":
            self.stdout.write(self.style.WARNING("seed_mediciti expects variant A. Skipping."))
            return
    ...
    def _patient(self, nid, demo):
        ...
        if not created:
            # keep rich fields up to date on re-run
