"""
Validate the FHIR resources this lab emits.

Runs local lab reports through the adapter and checks each emitted resource
against a minimal FHIR R4 structural contract (required fields per resourceType).
This proves the adapter output is standards-shaped BEFORE the National Platform
ever fetches it.
...
from lab import fhir
from lab.models import LabPatient, LabReport
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        nids = ([options["nid"]] if options.get("nid")
                else list(LabPatient.objects.values_list("nid", flat=True).distinct()))
        if not nids:
        ...
        for nid in nids:
            ...
            for r in LabReport.objects.filter(patient__nid=nid).prefetch_related("results"):
                res = fhir.to_fhir_diagnostic_report(r)
                total += 1
                rtype = res.get("resourceType", "?")
                counts[rtype] = counts.get(rtype, 0) + 1
                for problem in self._check(res):
                ...
                # Check contained Observations
                for obs in res.get("contained", []):
                    total += 1
                    rtype_obs = obs.get("resourceType", "?")
                    counts[rtype_obs] = counts.get(rtype_obs, 0) + 1
                    for problem in self._check(obs):
                        errors += 1
                        self.stdout.write(self.style.ERROR(f"  [{nid}] {rtype_obs} (contained): {problem}"))
    ...
    def _check(self, res):
        ...
        # Check subject/patient has identifier with value (NID)
